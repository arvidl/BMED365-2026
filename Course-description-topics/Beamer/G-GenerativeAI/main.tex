...
% =====================================================================
% SEKSJON: Introduction to Generative AI
% =====================================================================
\section{Introduction to Generative AI}

\begin{frame}{G01: Define generative AI og skille fra diskriminativ AI}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \textbf{Diskriminativ AI:}
            \begin{itemize}
                \item Lærer å \textbf{klassifisere} or skille mellom kategorier
                \item Modorer $P(y|x)$ -- sannsynlighet for klasse $y$ gitt input $x$
                \item Examples: ``Er dette kreft?'', ``Hvilken sykdom?''
        ...
        \begin{column}{0.48\textwidth}
            ...
            \begin{itemize}
                ...
                \item Modorer $P(x)$ or $P(x|y)$ -- datafordelingen selv
                \item Examples: Tekst, bilder, kode, musikk
                \item LLM, diffusjonsmodor
            \end{itemize}
        \end{column}
    \end{columns}
    ...
    \begin{alertblock}{\footnotesize Medical application}
        \footnotesize Generative AI: Oppsummere journaler, generere rapporter, svare på spørsmål, syntetisere trainingsdata
    \end{alertblock}
\end{frame}

% =====================================================================
% SEKSJON: Transformer Architecture
% =====================================================================
\section{Transformer Architecture}

\begin{frame}{G02: Explain \href{https://en.wikipedia.org/wiki/Attention_(machine_learning)}{self-attention}-mekanismen på et konseptuelt nivå}
...
\begin{frame}{G03: Describe transformer-arkitekturen (\href{https://en.wikipedia.org/wiki/Transformer_(deep_learning_architecture)\#Encoder}{encoder}-\href{https://en.wikipedia.org/wiki/Transformer_(deep_learning_architecture)\#Decoder}{decoder})}
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.42\textwidth}
            ...
            \begin{block}{\footnotesize GPT = ``Decoder-only''}
                \footnotesize Store language models (LLM) bruker typisk kun decoder-delen for tekstgenerering.
            \end{block}
        \end{column}
    \end{columns}
\end{frame}
...
% =====================================================================
% SEKSJON: LLM Fundamentals
% =====================================================================
\section{LLM Fundamentals}

\begin{frame}{G04: Explain hva \href{https://en.wikipedia.org/wiki/Lexical_analysis\#Token}{tokens} er og hvordan \href{https://en.wikipedia.org/wiki/Tokenization_(lexical_analysis)}{tokenisering} fungerer}
    ...
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \textbf{\href{https://en.wikipedia.org/wiki/Byte_pair_encoding}{BPE} og språkforskjor:}
            \footnotesize
            \begin{itemize}
        ...
        \begin{column}{0.48\textwidth}
            ...
            \begin{itemize}
                ...
                \item Ulike modor = ulike tokenizers
            \end{itemize}
        \end{column}
    \end{columns}

    \begin{block}{\footnotesize Why tokens?}
        \footnotesize Mer effektivt enn bokstaver (for kort) or hele ord (vokabular for stort).
    \end{block}
\end{frame}

\begin{frame}{G05: Understand konseptet \href{https://en.wikipedia.org/wiki/Context_window}{kontekstvindu} (context window)}
...
\begin{frame}{G10: Describe god praksis for \href{https://en.wikipedia.org/wiki/Prompt_engineering\#System_prompts}{systemprompts}}
    ...
    \begin{enumerate}
        ...
        \item \textbf{Limitations:} ``Ikke gi diagnoser...''
...
\begin{frame}{G15: Fra \href{https://en.wikipedia.org/wiki/Prompt_engineering}{Prompt Engineering} til \href{https://www.anthropic.com/research/building-effective-agents}{Context Engineering}}
    ...
    \begin{itemize}
        \item \textbf{\href{https://en.wikipedia.org/wiki/Prompt_engineering}{Prompt engineering}:} Fokus på å utforme gode spørsmål
    ...
    \begin{tabular}{lll}
        \toprule
        \textbf{Komponent} & \textbf{Describelse} & \textbf{Medical example} \\
        \midrule
    ...
    \normalsize
    \vspace{0.15cm}
    \begin{block}{\footnotesize Why viktig i medisin?}
        \footnotesize Rik kontekst $\rightarrow$ sikrere, mer relevante og personaliserte svar
    \end{block}
\end{frame}

\begin{frame}{G16--G17: Modellspesifikke prompting-teknikker}
    \textbf{Tre ledende LLM-leverandører med ulike styrker:}
...
\begin{frame}{G18: Håndtere usikkerhet og hallusinasjonsrisiko}
    \textbf{Prompt-mønster for medisinsk usikkerhetshåndtering:}
    
    \vspace{0.2cm}
    \begin{block}{\footnotesize Anti-hallucination prompt (GPT-5.2 stil)}
        \scriptsize
        \texttt{<usikkerhet\_og\_tvetydighet>} \\
        \texttt{- Ved tvetydige symptomer: Presenter differensialdiagnoser med sannsynlighet} \\
        \texttt{- Oppgi tydelig kunnskapens begrensninger og dato for oppdatering} \\
        \texttt{- Henvis alltid til gjeldende retningslinjer og klinisk vurdering} \\
        \texttt{- Ved usikkerhet: Eshallér til høyere hastegrad} \\
        \texttt{</usikkerhet\_og\_tvetydighet>}
    \end{block}
    
    \vspace{0.15cm}
    \textbf{Selvsjekk for høyrisiko-oppgaver:}
    \footnotesize
    \begin{enumerate}
    ...
    \begin{alertblock}{\footnotesize Rule of thumb}
        \footnotesize AI = beslutningsstøtte, \textbf{ikke} beslutningstaker. Alltid klinisk validation!
    \end{alertblock}
\end{frame}

\begin{frame}{G19--G20: Strukturert ekstraksjon og modellsammenligning}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
        ...
        \begin{column}{0.48\textwidth}
            ...
            \footnotesize
            \begin{enumerate}
                \item Test samme prompt på 2--3 modor
                \item Sammenlign mot evaluationskriterier
                \item Velg modell basert på oppgave
...
% =====================================================================
% SEKSJON: Challenges with LLM
% =====================================================================
\section{Challenges with LLM}

\begin{frame}{G11: Define \href{https://en.wikipedia.org/wiki/Hallucination_(artificial_intelligence)}{hallusinering} og dens implikasjoner}
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.42\textwidth}
            ...
            \footnotesize
            \begin{itemize}
                \item Nyere modor hallusinerer \textit{mindre}
...
% =====================================================================
% SEKSJON: Models and Advanced Concepts
% =====================================================================
\section{Models and Advanced Concepts}

\begin{frame}{G12: Know about GPT-5, Claude, Gemini og deres applylser}
    \begin{center}
        \scriptsize
...
\begin{frame}{G13: Explain konseptet \href{https://en.wikipedia.org/wiki/Foundation_model}{grunnmodell (foundation model)}}
    ...
    \begin{enumerate}
        \item \textbf{Stor shalla:} Milliarder av parametre
        \item \textbf{Selvveiledet læring:} Ingen labels nødvendig
        \item \textbf{Emergente egenskaper:} Evner som ``dukker opp'' ved shalla
