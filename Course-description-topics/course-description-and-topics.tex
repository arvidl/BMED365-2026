...
\subsection{Why AI in Biomedical and Health-related Contexts?}
...
\begin{itemize}[nosep]
    ...
    \item Explain principles and applications in computational imaging and modeling
...
\begin{itemize}[nosep]
    \item Assess opportunities and limitations of AI in clinical practice and biomedical research
...
\begin{table}[htbp]
\centering
\caption{Schedule BMED365 -- January-March 2026}
\label{tab:schedule}
\begin{tabular}{@{}cllll@{}}
\toprule
...
10 & Fri 06.03 & 09:00--11:00 & \textbf{HOME EXAM} & Inspera \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{description}
    ...
    \item[MittUiB:] \url{https://mitt.uib.no/courses/50785} (enrolled only)
\end{description}
...
\begin{tikzpicture}[
    ...
    box/.style={rectangle, draw=uibblue, fill=uibblue!10, 
        text width=2.5cm, text centered, rounded corners, minimum height=0.9cm, font=\small},
    box2/.style={rectangle, draw=uibblue!70!black, fill=uibblue!20, 
        text width=2.5cm, text centered, rounded corners, minimum height=0.9cm, font=\small},
    arrow/.style={thick,->,>=stealth,uibblue}
]
    % Block 1 label
    \node[font=\footnotesize\bfseries, text=uibblue] at (-1.3,0.4) {Block 1};
    
    % Row 1: Labs 0-2
    \node[box] (quickstart) {Quick Start\\{\footnotesize AI-Python}};
    \node[box, right=of quickstart] (lab0) {Lab 0\\{\footnotesize Machine Learning}};
    \node[box, right=of lab0] (lab1) {Lab 1\\{\footnotesize Networks}};
    \node[box, right=of lab1] (lab2) {Lab 2\\{\footnotesize Deep Learning}};
    
    % Row 2: Lab 3
    \node[box, below=1cm of lab0, xshift=1.3cm] (lab3) {Lab 3\\{\footnotesize Generative AI}};
    
    % Row 3: Team Project
    \node[box, below=1cm of lab3] (team) {Team Project\\{\footnotesize Glioblastoma}};
    
    % Block 2 label
    \node[font=\footnotesize\bfseries, text=uibblue!70!black] at (10,0.4) {Block 2};
    
    % Block 2: Labs 4-5 (right side)
    \node[box2, right=1.2cm of lab2] (lab4) {Lab 4\\{\footnotesize Comp.\ Imaging}};
    \node[box2, below=1cm of lab4] (lab5) {Lab 5\\{\footnotesize Comp.\ Modeling}};
    
    % Individual Project
    \node[box2, below=1cm of lab5] (indiv) {Individual Project\\{\footnotesize Poster + Presentation}};
    
    % Row 5: Exam (centered)
    \node[box, below=1cm of team, xshift=2.2cm, fill=uibred!10, draw=uibred] (exam) {Exam\\{\footnotesize MCQ + Essay}};
    ...
    % Block 2 Arrows
    \draw[arrow] (lab2) -- (lab4);
    \draw[arrow] (lab4) -- (lab5);
    \draw[arrow] (lab5) -- (indiv);
    \draw[arrow] (indiv) |- (exam);
\end{tikzpicture}%
}
\caption{Course structure and progression in BMED365 (Block 1 and Block 2)}
...
\begin{tikzpicture}[
    ...
    level 1/.append style={level distance=4cm, sibling angle=51.4},
    level 2/.append style={level distance=2.5cm, sibling angle=45}
]
    \node[concept] {AI in\\Biomedicine}
        ...
        child [concept color=cyan!40] { node {Comp.\\Imaging}
            child { node {MRI/IMC} }
            child { node {Segmentation} }
        }
        child [concept color=teal!40] { node {Comp.\\Modeling}
            child { node {ODEs} }
            child { node {Physiology} }
        };
\end{tikzpicture}
